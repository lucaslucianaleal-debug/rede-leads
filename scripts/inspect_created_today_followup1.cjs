...
async function inspectDoc(docPath, label) {
  ...
  const matched = leads.filter(l => (l.dataCriacao || '').startsWith(todayFormatted) && (l.etapaLead || '').startsWith('Follow-Up'));
  console.log(`${label}: found ${matched.length} leads created today with etapaLead starting 'Follow-Up' (doc: ${docPath})`);
  matched.forEach(l => {
    console.log(JSON.stringify({ id: l.id, followUpCount: l.followUpCount, dataCriacao: l.dataCriacao, etapaLead: l.etapaLead, dataFollowUp: l.dataFollowUp, observacao: l.observacao }, null, 2));
  });
}
