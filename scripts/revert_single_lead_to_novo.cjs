...
(async () => {
  try {
    ...
    const id = 'lead_1773678801047';

    // backup pre-change
    const backupsDir = path.resolve(__dirname, '..', 'backups', 'reverts', `single-revert-${Date.now()}`);
    ...
    if (changed === 0) {
      console.log('No lead needed reverting.');
      process.exit(0);
    }
    ...
    console.log(`Reverted lead ${id} in ${docPath}. Backups written to ${backupsDir}`);
  } catch (err) {
