...
export function ClinicFinanceDashboardV2() {
  ...
  }, [imported, patients, search, selectedBucket]);
    ...
    <div className="space-y-5">
      ...
      <section className="flex flex-col gap-4 rounded-2xl border bg-card p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button size="sm" variant={view === "delinquency" ? "default" : "ghost"} onClick={() => { setView("delinquency"); if (!selectedBucket) setSelectedBucket(null); }}>Cobrança</Button>
        </div>
      </section>
  ...
  return (
    <Card>
      <CardHeader className="border-b pb-4">
        ...
        {!imported && <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800">Importe o relatório para carregar a fila real de pacientes.</div>}
      </CardHeader>
      <CardContent className="overflow-x-auto p-0">
        <table className="w-full min-w-[980px] text-sm">
          ...
          <tbody>
            ...
            {!queue.length && <tr><td colSpan={8} className="px-4 py-10 text-center text-muted-foreground">{imported ? "Nenhum paciente encontrado nesta faixa." : "Importe o PDF para carregar os pacientes."}</td></tr>}
          </tbody>
        </table>
      </CardContent>
    </Card>
