...
export function ClinicFinanceDailyDesk() {
  ...
  return <>
    <section className="space-y-3"><div><h2 className="text-lg font-bold">Mesa de cobrança de hoje</h2><p className="text-sm text-muted-foreground">Abra o Financeiro e veja imediatamente o que precisa ser trabalhado agora.</p></div><div className="grid gap-3 lg:grid-cols-3">
    ...
    {mode&&<div className="fixed inset-0 z-[95] bg-black/45 p-3 sm:p-6"><div className="mx-auto flex h-full max-w-[1200px] flex-col overflow-hidden rounded-2xl bg-background shadow-2xl"><div className="flex items-center justify-between border-b p-4"><div><h3 className="text-xl font-bold">{mode==="today"?"Cobranças com vencimento hoje":mode==="tomorrow"?"Lembretes de amanhã":"Recuperação de contas — fila do dia"}</h3><p className="text-sm text-muted-foreground">{activeRows.length} paciente(s) • {pendingCount(activeRows)} ainda pendente(s)</p></div><Button size="icon" variant="ghost" onClick={()=>setMode(null)}><X className="h-5 w-5"/></Button></div><div className="flex-1 overflow-auto divide-y">
      {activeRows.map((row)=>{const phone=row.patient.phones[0]||"";const amount=mode==="recovery"?row.overdueAmount:row.amount;return <div key={row.patient.id} className={`grid gap-3 p-4 md:grid-cols-[1fr_190px_220px] md:items-center ${row.action?"bg-emerald-50/40":""}`}><div><div className="font-semibold">{row.patient.name}</div><div className="mt-1 text-xs text-muted-foreground">{phone||"Sem telefone"} • {row.open.length} parcela(s) em aberto{row.overdue.length?` • ${row.overdue.length} vencida(s)`:""}</div>{row.overdue.length>1&&<div className="mt-1 text-xs font-medium text-red-700">A mensagem já aborda negociação do conjunto de débitos.</div>}</div><div><div className="font-semibold">{brl.format(amount)}</div><div className="text-xs text-muted-foreground">{mode==="recovery"?`${row.oldest} dias de maior atraso`:`${row.due.length} parcela(s) desta fila`}</div></div><div className="flex justify-end gap-2">{row.action?<span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5"/>Trabalhado hoje</span>:<><Button size="sm" variant="outline" className="gap-2" onClick={()=>openWhatsapp(row,mode)} disabled={!phoneDigits(phone)}><MessageCircle className="h-4 w-4"/>Abrir WhatsApp</Button><Button size="sm" onClick={()=>void markDone(row,mode)}>Marcar feito</Button></>}</div></div>;})}
