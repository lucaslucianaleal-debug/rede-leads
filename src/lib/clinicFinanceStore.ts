...
export function readLocalFinanceStore(clinicId: string): FinanceStore | null {
  try {
    const raw = localStorage.getItem(financeStoreKey(clinicId));
    if (!raw) return null;
...
export function writeLocalFinanceStore(clinicId: string, store: FinanceStore) {
  localStorage.setItem(financeStoreKey(clinicId), JSON.stringify(store));
}
...
export async function readRemoteFinanceStore(clinicId: string): Promise<FinanceStore | null> {
  ...
  const patients = patientsSnap.docs
    .map((snapshotDoc) => snapshotDoc.data() as FinancePatient)
...
export async function hydrateFinanceStore(clinicId: string) {
  const [remote, local] = await Promise.all([
    readRemoteFinanceStore(clinicId),
    Promise.resolve(readLocalFinanceStore(clinicId)),
  ]);

  const canonical = mergeFinanceStores(remote, local);
